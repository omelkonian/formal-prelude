...
--   * Equivalence assuming same domain
------------------------------------------------------------------------
...
module Prelude.MapsSets2 {K V : Set} {{_ : DecEq K}} where
...
_⊈ᵈ_ = _⊈′_ on keys
_♯_ = S._♯_ on keys
m ≈ m′ = ∀ {k} (k∈ : k ∈ᵈ m) (k∈′ : k ∈ᵈ m′) → lookup m k∈ ≡ lookup m′ k∈′
  -- ∀ k → (m ⁉ k) ≡ (m′ ⁉ k)
...
≈-refl : s ≈ s
≈-refl {s = s} k∈ k∈′ = cong (lookup s) (∈′-irr {Xs = keys s} k∈ k∈′)

≈-trans : s₁ ≈ s₂ → s₂ ≈ s₃ → s₁ ≈ s₃
≈-trans p q {k} k∈ k∈′ = {!!} -- trans (p ) (q k)
